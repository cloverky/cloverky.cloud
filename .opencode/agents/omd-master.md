...
- Resolve referenced OmD skills from `.opencode/skills/<skill>/SKILL.md`.
- Resolve the installed offline catalog from `.opencode/data`.
...
Status values:
- `ask_user` — launcher presents questions from `questions_file` to the user, saves answers.json, re-spawns master
...
host question-interface compatible. 1-4 questions per checkpoint. Examples:
